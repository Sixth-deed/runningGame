 ...
 * - is_an_Entity: Indicates whether the object is an entity.
 *
...
protected:
    ...
    static const EntityType etype = EntityType::PassiveMoveEntity;
    constexpr EntityType getEntityType() const { return etype; }
public:
    ...
    const gMath::tVector &getVelocity() const
    {
        return velocity;
    }
...
{
protected:
    LiberalPhysicsObj() : PhysicsObj(), RotateObj(), MoveObj(){}
    static const EntityType etype = EntityType::PassiveMoveEntity;
    constexpr EntityType getEntityType() const { return etype; }
public:
    /**
    ...
    */
                        ...
                        const gMath::tVector &initialVelocity = gMath::tVector(0.0, 0.0), const gMath::tVector &initialAccelr = gMath::tVector(0.0, 0.0))
    {
        ...
        MoveObj::initObj(t, crd, angle_, initialVelocity, initialAccelr);
        RotateObj::initObj(t, crd, angle_, angleV, angleA);
        t->Inertia = inertia_;
        t->inverseInertia = 1 / inertia_;
    }
    /*
    template <typename managerT>
    static LiberalPhysicsObj &newObj(managerT &m, const gMath::Crdinate &crd, gMath::Angle angle_, clsn::CollisionBox &cl,
...
inline void LiberalPhysicsObj::Integrate(double dt)
{
    ...
    rotated = angleVelocity >= MIN_ROTATION_VECLOCITY_COUNT;

    if (toSleep && !moved && !rotated)
    {
        setSleep();
    }
}
